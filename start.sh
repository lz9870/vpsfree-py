...
async def generate_links(argo_domain):
    meta_info = subprocess.run(['curl', '-s', 'https://speed.cloudflare.com/meta'], capture_output=True, text=True)
    meta_info = meta_info.stdout.split('"')
    ISP = f"{meta_info[25]}-{meta_info[17]}".replace(' ', '_').strip() if len(meta_info) > 25 else "Unknown"

    time.sleep(2)
    
    # 确保 argo_domain 有值，否则使用 CFIP 作为 fallback
    argo_domain = argo_domain if argo_domain else CFIP
...
{TROJAN_80}
    ...
    print(f"Generated subscription:\n{sub_txt}")
    print(f"{FILE_PATH}/sub.txt saved successfully")
...
# 启动app.py
screen -dmS xray_argo python3 app.py

echo -e "${GREEN}服务已启动！使用 screen -r xray_argo 查看日志${NC}"

# 保存节点信息（假设您有逻辑来生成NODE_INFO_FILE，这里省略具体实现，根据需要添加）
